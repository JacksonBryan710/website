...
The `refresh-feed-cache` Edge Function is scheduled (Supabase Cron), not user-triggered — it only accepts the project's secret key. It fetches the Letterboxd diary RSS feed and two Goodreads shelf RSS feeds directly (not through a third-party proxy — rss2json's own bot protection was blocking Supabase's shared Edge IP range), parses them, and rewrites `feed_cache` using the `service_role` key, which bypasses RLS. `service_role` grants in migrations exist specifically to support this — see `.coderabbit.yaml` for how PR review treats them differently from `anon`/`authenticated` grants.
